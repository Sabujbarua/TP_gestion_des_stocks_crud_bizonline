...
var bills = (function bills() {
    ...
    function addBillInDOMList(insertedId, billData) {
      ...
      td.textContent = insertedId;//id de la facture de renouvellement inserée
      tr.appendChild(td);
      td = document.createElement("td");
      ...
      tr.appendChild(td);
      td = document.createElement("td");//creaion du td pour editer la facture
      td.className = "update";
      td.innerHTML = "<span class=\"tabler-btn\">edit</span>";
      td.querySelector(".tabler-btn").onclick = paramEditForm;
      tr.appendChild(td);
      td = document.createElement("td");//creation du td de suppression
      td.className = "delete";
      ...
      //cette fonction retourne le tr entier que l'on vient de créer
    ...
    function createBill(evt, mode) {
      ...
      if(createdAt && total) {
        ...
        xhr.onload = function getServerResponse(){
          //on le nomme comme on veut ici, id ca peut etre aussi toto ou autre
          //ici on fait reference au lastInsertId qui a ete inserer
          if(mode === "create"){
    ...
    function deleteBills() {
      ...
      if(checked.length) {
        ...
        checked.forEach(function(c) {
          ...
          idsBills.push(idBill);//idBills est accessible dans tous le scope du mo
        });
    ...
    function observer() {
        ...
        if (formBillBtn) formBillBtn.onclick = function chooseFormMode(){
          if(formStatus === "create") createBill();
          else updateBill();
        };
    ...
    function openForm(billToEdit) {
      ...
      if(formBill.classList.contains("is-active")){
        ...
        if(billToEdit){
          ...
          formStatus = "create";
          formBill.value = "Ajouter facture";
        }
      }
    ...
    function paramEditForm(evt) {
      //autre facon d'ecrire avec this, on retire la 1ere ligne et on remplace src par this
      const src = evt.target || evt.srcElement;
      const parent = src.parentElement.parentElement;
      const fd = new FormData();
      const xhr = new XMLHttpRequest();
      ...
      xhr.onload = function getBillFromServer(){
        //ici this est la reponse qui est xhr, thsi est l'evnt de la source qui est onload
    ...
    function removeBillsFromDOMList() {
      ...
      //parse est la nomination de la fonction on peut meme ne pas la nommée
      checkboxes.forEach(function parse(checkbox) {
        //remonte sur les tr parents et ....
        //retire un à un ces élèments du document
